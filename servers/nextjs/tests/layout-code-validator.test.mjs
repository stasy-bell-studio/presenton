...
test("rejects syntax errors with a source location", async () => {
...
test("rejects malformed JSX tags", async () => {
  ...
  assert.throws(
    () =>
      validateLayoutCode(
        validCode.replace("<div>{data.title}</div>", "<div><span></div>")
      ),
    (error) =>
      error instanceof LayoutCodeValidationError &&
...
test("rejects missing or computed layout metadata", async () => {
  const { LayoutCodeValidationError, validateLayoutCode } = await validatorPromise;
  const missingName = validCode
    .replace('const layoutName = "Valid Layout";', "")
    .replace("layoutName, ", "");
  const computedName = validCode.replace(
    'const layoutName = "Valid Layout";',
    'const layoutName = ["Valid", "Layout"].join(" ");'
  );

  assert.throws(
    () => validateLayoutCode(missingName),
    (error) =>
      error instanceof LayoutCodeValidationError &&
      error.message === "Layout code must declare layoutName"
  );
  assert.throws(
    () => validateLayoutCode(computedName),
    (error) =>
      error instanceof LayoutCodeValidationError &&
      error.message ===
        "Layout metadata layoutName must be a non-empty string literal"
  );
});
